...
// 9: tail
$rope = [];
for ($ropeIndex=0; $ropeIndex<10; $ropeIndex++) {
	$rope[] = new Knot();
}
...
foreach ($moveLines as $moveLine) {
	...
	for ($step = 0; $step < $stepCount; $step++) {
		...
		// Let all other knots follow
		for ($ropeIndex=1; $ropeIndex<count($rope); $ropeIndex++) {
			$rope[$ropeIndex]->follow($rope[$ropeIndex-1]);
		}

		$tailVisited[(string)$rope[count($rope)-1]] = true;
		//echo "        Head: ".$head."\n";
		//echo "        Tail: ".$tail."\n";
	}
}
